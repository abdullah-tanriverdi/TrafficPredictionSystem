...
from services.routing import calculate_route, get_turn_by_turn
...
def generate_route():
    ...
    # Basitleştirilmiş toplam mesafe ve zaman hesaplama
    total_length_m = sum(geodesic(route_coords[i], route_coords[i+1]).meters for i in range(len(route_coords)-1))
    average_speed_kmh = 30  # Ortalama hız (örnek)
    total_time_min = round((total_length_m / 1000) / average_speed_kmh * 60, 1)


    osrm_route, osrm_directions = get_osrm_route((start_lat, start_lon), (end_lat, end_lon))
    if osrm_route:
